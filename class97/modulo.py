...
def calcula_digito(cnpj, digito):   # faz todos os calculos e checagens de REGRESSIVO
    if digito == 1:
...
def eh_sequencia(cnpj): # checa se a CNPJ é uma sequencia ou não
...
def apenas_numeros(cnpj):   # remove da str tudo que não esta entre 0 e 9
    return re.sub(r'[^0-9]', '', cnpj)


def gera(): # gera uma cnpj nova
...
def formata(cnpj): # formata a cnpj 
    cnpj = apenas_numeros(cnpj)
